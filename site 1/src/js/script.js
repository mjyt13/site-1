// вешать на овно прослушку загрузки DOM-дерева
// по окончании загрузки будет запущена функция
window.onload = () => {
    //найти ноды - элементы DOM-дерева - для их получения ввиду необходимости последующий манипуляций
    ...
    const updateBurgerState = () =>{
        ...
        if(width <= 1024 || isMobileBrowser) return showBurger();

        return hideBurger();
    };

    //resize окна на старте для получения корректного состояния меню при загрузке страницы
    updateBurgerState();
    ...
    // добавить прослушку клика на бургер
    ...
    // повесить прослушку на закрытие меню
